...
class PlatformController extends CDUAbstractController {

    private $mRouter = ['GET' => 'get_platform', 'POST' => 'post_platform'];
    private $uniqueFields = array('name');
    private $privateKey = 'id';
    private $validateForm = ['name'=>'required|max:255','type' => 'required|max:255'];
    private $pagingNumber = 10;
    private $validateMaker;
    public function __construct(){
        $this->validateMaker = Validator(array(),array(),array());
        parent::__construct(new Platform(),$this->privateKey,$this->uniqueFields,$this->validateForm,null,$this->validateForm);
    }
    ...
    public function returnView($data){
        ...
        $view = view('admin/setting/platform.platformIndex',['router' => $this->mRouter,'listData'=>$listData,
